...
while IFS= read -r rsrc_fn; do
    ...
    (export PYTHONPATH=".."; ../pylabview/readRSRC.py -vv -x --keep-names -i "${rsrc_fn}" -m "${single_vi_path}/${xml_fn}") 2>&1 | tee "${log_fn}"
    cat "${log_fn}" | tee -a log-vi_lib-llb-1extr.txt

    (export PYTHONPATH=".."; ../pylabview/readRSRC.py -vv -c -m "${single_vi_path}/${xml_fn}" -i "${rsrc_out_fn}") 2>&1 | tee -a log-vi_lib-llb-2creat.txt

    # Re-extract - we can only compare extracted files
    (export PYTHONPATH=".."; ../pylabview/readRSRC.py -vv -x --keep-names -i "${rsrc_out_fn}" -m "${xml_fn}") 2>&1 | tee -a log-vi_lib-llb-3reext.txt
